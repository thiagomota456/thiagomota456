# Hi, I'm Thiago Soares Mota

### **Full Stack Software Engineer**

> Backend engineer focused on **system integration, APIs, and legacy platform modernization.**

<p align="left">
  <img src="https://img.shields.io/badge/.NET-512BD4?style=for-the-badge&logo=dotnet&logoColor=white" alt="dotnet" />
  <img src="https://img.shields.io/badge/Java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white" alt="java" />
  <img src="https://img.shields.io/badge/Spring_Boot-6DB33F?style=for-the-badge&logo=spring-boot&logoColor=white" alt="spring" />
  <img src="https://img.shields.io/badge/Angular-DD0031?style=for-the-badge&logo=angular&logoColor=white" alt="angular" />
</p>

---

## About Me

I am a software engineer with extensive experience in the industrial and administrative integration sectors. Currently, I focus on:

* **Automation:** Developing new products to automate and simplify complex business processes.
* **Quality:** Ensuring high reliability and maintainability in every line of code.
* **Modernization:** Helping companies transition from legacy systems to modern, scalable architectures.

---

## Tech Stack

### **Backend**

* **Languages:** C#, Java, Kotlin
* **Frameworks:** ASP.NET Core, Spring Boot
* **Architecture:** RESTful APIs, Microservices, System Integration

### **Frontend & Database**

* **Web:** Angular, JavaScript, TypeScript
* **Databases:** PostgreSQL, MySQL, SQL Server

---

## My Professional Journey

I have a strong background in creating software solutions that bridge the gap between industrial operations and administrative management. My goal is always to deliver software that is not just functional, but sustainable and easy to scale.

**View my full professional experience on LinkedIn:** [linkedin.com/in/thiagosoaresmota](https://www.linkedin.com/in/thiagosoaresmota)

---

## Let's Connect!

* **Email:** [thiagomota456@gmail.com](mailto:thiagomota456@gmail.com)
* **LinkedIn:** [Thiago Soares Mota](https://www.linkedin.com/in/thiagosoaresmota)

* **Email:** [thiagomota456@gmail.com](mailto:thiagomota456@gmail.com)
* **LinkedIn:** [Thiago Soares Mota](https://www.linkedin.com/in/thiagosoaresmota)
